...
def doKey(key):
  ...
  if key=='#':
      ...
      if code.endswith('#'):
        ...
        disp.display()
        draw.rectangle((0,0,width,height), outline=0, fill=0)   
        draw.text((x, top),        'Employee no. :' ,  font=font, fill=255)
        draw.text((x, top + 8 ),        code ,  font=font, fill=255)
        # draw.text((x, top + 36),        'Is the No. ok ?' ,  font=font, fill=255)
        draw.text((x, top + 16),        'press * to proceed' ,  font=font, fill=255)
        draw.text((x, top + 24),        'press 0 to retry' ,  font=font, fill=255)
...
def finger():
    ...
    while 1:
        ...
        if ( positionNumber >= 0 ):
            ...
            disp.display()
            draw.rectangle((0,0,width,height), outline=0, fill=0)   
            draw.text((x, top + 16),'Finger already exists' ,  font=font, fill=255)
            disp.image(image)
            disp.display()
            time.sleep(1)
            os.system("python password.py")
...
while 1:
    ...
    disp.display()
    draw.rectangle((0,0,width,height), outline=0, fill=0)   
    draw.text((x, top),       'Please enter employee' ,  font=font, fill=255)
    draw.text((x, top + 8),        'no:' ,  font=font, fill=255)
    draw.text((x, top + 16),        'Press # after' ,  font=font, fill=255)
    draw.text((x, top + 24),        'Completion' ,  font=font, fill=255)  
    disp.image(image)
    disp.display()
